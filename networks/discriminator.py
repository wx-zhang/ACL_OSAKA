...
class Discriminator(torch.nn.Module):
    def __init__(self,args,domain_id):
        super(Discriminator, self).__init__()

        self.d_units=args.d_units
        ...
        if args.diff == 'yes':
            self.dis = torch.nn.Sequential(
                ...
                torch.nn.Linear(self.hidden_size, args.d_units),
                torch.nn.LeakyReLU(),
                torch.nn.Linear(args.d_units, args.d_units),
                torch.nn.Linear(args.d_units, domain_id + 2)
            )
        else:
            self.dis = torch.nn.Sequential(
                torch.nn.Linear(self.hidden_size, args.d_units),
    ...
    def forward(self, z, labels, domain_id):
        return self.dis(z)

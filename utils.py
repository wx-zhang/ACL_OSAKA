...
def report_tr(res, e, sbatch, clock0, clock1):
    ...
    print(
        '| Epoch {:3d}, time={:5.1f}ms/{:5.1f}ms | Train losses={:.3f} | T: loss={:.3f}, r2={:5.5f} | D: loss={:.3f}, acc={:5.1f}%, '
        'Diff loss:{:.3f} |'.format(
            e + 1,
            1000 * sbatch * (clock1 - clock0) / res['size'],
            1000 * sbatch * (time.time() - clock1) / res['size'], res['loss_tot'],
            res['loss_t'], res['r2'], res['loss_a'], res['acc_d'], res['loss_d']), end='')

def report_val(res):
    # Validation performance
    print(' Valid losses={:.3f} | T: loss={:.6f}, r2={:5.2f}, | D: loss={:.3f}, r2={:5.5f}, Diff loss={:.3f} |'.format(
        res['loss_tot'], res['loss_t'], res['r2'], res['loss_a'], res['acc_d'], res['loss_d']), end='')



def log_wandb(res,e,mode):
    import wandb
    if mode == 'pre-train_train':
        wandb.log({'Pre-train_training_task_loss': res['loss_t'],'epoch':e})
        wandb.log({'Pre-train_training_task_measure': res['acc_measure'],'epoch':e})
        wandb.log({'Pre-train_training_total_loss': res['loss_tot'],'epoch':e})
        wandb.log({'Pre-train_training_adversarial_loss': res['loss_a'],'epoch':e})
        wandb.log({'Pre-train_training_orthogonal_loss': res['loss_d'],'epoch':e})
        wandb.log({'Pre-train_training_discriminator_accuracy': res['acc_d'],'epoch':e})
    elif mode == 'pre-train_valid':
        wandb.log({'Pre-train_valid_task_loss': res['loss_t'],'epoch':e})
        wandb.log({'Pre-train_valid_task_measure': res['acc_measure'],'epoch':e})
        wandb.log({'Pre-train_valid_total_loss': res['loss_tot'],'epoch':e})
        wandb.log({'Pre-train_valid_adversarial_loss': res['loss_a'],'epoch':e})
        wandb.log({'Pre-train_valid_orthogonal_loss': res['loss_d'],'epoch':e})
        wandb.log({'Pre-train_valid_discriminator_accuracy': res['acc_d'],'epoch':e})
    elif mode == 'pre-train_test':
        wandb.log({'Pre-train_test_task_loss': res['loss_t']})
        wandb.log({'Pre-train_test_task_measure': res['acc_measure']})
        wandb.log({'Pre-train_test_total_loss': res['loss_tot']})
        wandb.log({'Pre-train_test_adversarial_loss': res['loss_a']})
        wandb.log({'Pre-train_test_orthogonal_loss': res['loss_d']})
        wandb.log({'Pre-train_test_discriminator_accuracy': res['acc_d']})
